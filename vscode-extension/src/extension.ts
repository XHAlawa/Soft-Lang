...
export function activate(context: vscode.ExtensionContext) {
            ...
            provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
                ...
                if (inTemplate) {
                    ...
                    if (inTag) {
                        ...
                        completions.push(
                            createAttribute('bind:value', 'Two-way data binding'),
                            createAttribute('bind:checked', 'Checkbox binding'),
                            createAttribute('click', 'Click event handler'),
                            createAttribute('submit', 'Submit event handler'),
                            createAttribute('input', 'Input event handler'),
                            createAttribute('class:active', 'Conditional class'),
                            createAttribute('style:color', 'Dynamic style'),
                            createAttribute('ref', 'Element reference')
                        );
                    }
                    // Always show directives in template
                    completions.push(
                        createDirective('if', 'Conditional rendering', '(condition) {'),
                        createDirective('foreach', 'Loop rendering', '(item in items) {'),
                        createDirective('switch', 'Switch statement', '(value) {'),
                        createDirective('case', 'Switch case', '("value") {'),
                        createDirective('default', 'Switch default case', ' {'),
                        createDirective('Navigate', 'Navigate to route', "('/path')")
                    );
                }
                ...
                if (!inTemplate) {
                    completions.push(
                        createDecorator('Page', 'Mark class as a page component'),
                        createDecorator('Service', 'Mark class as an injectable service'),
                        createDecorator('Route', 'Define route for page', '("/path")'),
                        createDecorator('Template', 'Define component template'),
                        createDecorator('Code', 'Define component code'),
                        createDecorator('Style', 'Define component styles'),
                        createDecorator('importComponent', 'Import another component', ' Name from "./path.s"'),
                        createDecorator('State', 'Reactive state property'),
                        createDecorator('Computed', 'Computed property'),
                        createDecorator('Watch', 'Watch property changes', "('property')"),
                        createDecorator('Prop', 'Component property')
                    );
                }
    ...
    const codeCompletionProvider = vscode.languages.registerCompletionItemProvider(
        'soft',
        {
            provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
                ...
                completions.push(
                    createTSKeyword('export', 'Export declaration'),
                    createTSKeyword('class', 'Class declaration'),
                    createTSKeyword('interface', 'Interface declaration'),
                    createTSKeyword('type', 'Type alias'),
                    createTSKeyword('function', 'Function declaration'),
                    createTSKeyword('const', 'Constant declaration'),
                    createTSKeyword('let', 'Variable declaration'),
                    createTSKeyword('var', 'Variable declaration'),
                    createTSKeyword('if', 'If statement'),
                    createTSKeyword('else', 'Else statement'),
                    createTSKeyword('for', 'For loop'),
                    createTSKeyword('while', 'While loop'),
                    createTSKeyword('return', 'Return statement'),
                    createTSKeyword('async', 'Async function'),
                    createTSKeyword('await', 'Await expression'),
                    createTSKeyword('try', 'Try block'),
                    createTSKeyword('catch', 'Catch block'),
                    createTSKeyword('throw', 'Throw statement'),
                    createTSKeyword('new', 'New instance'),
                    createTSKeyword('this', 'This reference'),
                    createTSKeyword('super', 'Super reference'),
                    createTSKeyword('public', 'Public modifier'),
                    createTSKeyword('private', 'Private modifier'),
                    createTSKeyword('protected', 'Protected modifier'),
                    createTSKeyword('readonly', 'Readonly modifier'),
                    createTSKeyword('static', 'Static modifier'),
                    createTSKeyword('true', 'Boolean true'),
                    createTSKeyword('false', 'Boolean false'),
                    createTSKeyword('null', 'Null value'),
                    createTSKeyword('undefined', 'Undefined value')
                );

                // Common TypeScript types
                completions.push(
                    createTSType('string', 'String type'),
                    createTSType('number', 'Number type'),
                    createTSType('boolean', 'Boolean type'),
                    createTSType('any', 'Any type'),
                    createTSType('void', 'Void type'),
                    createTSType('never', 'Never type'),
                    createTSType('unknown', 'Unknown type'),
                    createTSType('Array', 'Array type'),
                    createTSType('Promise', 'Promise type'),
                    createTSType('Date', 'Date type'),
                    createTSType('RegExp', 'RegExp type')
                );
    ...
    const changeListener = vscode.workspace.onDidChangeTextDocument(async (event) => {
        if (event.document.languageId !== 'soft') return;
        ...
        if (selfClosing.includes(tagName.toLowerCase())) return;
        
        // Check if already has closing tag or is self-closing
        if (beforeCursor.endsWith('/>') || beforeCursor.match(/\/\s*>$/)) return;
    ...
    const interpolationCompletionProvider = vscode.languages.registerCompletionItemProvider(
        'soft',
        {
            provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
                const textBefore = document.getText(new vscode.Range(0, 0, position.line, position.character));
    ...
    context.subscriptions.push(
        ...
        interpolationCompletionProvider,
        cssCompletionProvider
    );
}

function createDecorator(name: string, description: string, suffix: string = ''): vscode.CompletionItem {
...
function createTSKeyword(name: string, description: string): vscode.CompletionItem {
    const item = new vscode.CompletionItem(name, vscode.CompletionItemKind.Keyword);
    item.detail = description;
...
function createTSMethod(name: string, description: string, suffix: string = ''): vscode.CompletionItem {
...
function createTSType(name: string, description: string): vscode.CompletionItem {
...
function extractClassMembers(document: vscode.TextDocument, textBefore: string): vscode.CompletionItem[] {
    const completions: vscode.CompletionItem[] = [];
    ...
    const objectsWithNested = new Set<string>();
...
function extractNestedProperties(fullText: string, objectName: string): vscode.CompletionItem[] {
    const completions: vscode.CompletionItem[] = [];
...
function createDirective(name: string, description: string, suffix: string = ''): vscode.CompletionItem {
    const item = new vscode.CompletionItem(name, vscode.CompletionItemKind.Keyword);
    item.detail = description;
    item.insertText = name + suffix;
    item.documentation = new vscode.MarkdownString(`**@${name}**\n\n${description}`);
...
function createAttribute(name: string, description: string, suffix: string = ''): vscode.CompletionItem {
...
function getDecoratorDocumentation(decorator: string): vscode.MarkdownString | undefined {
