class SoftRouter {
    ...
    register(pattern, component, options = { canonical: true }) {
    ...
    navigate(pattern, params = {}, query = {}, options = {}) {
        ...
        if ('replace' in options || 'state' in options) {
            ...
            navigationOptions = { state: options };
        }
    ...
    replace(pattern, params = {}, query = {}, state = {}) {
        this.navigate(pattern, params, query, { replace: true, state });
    }
    handleNavigation(fullPath, fromNavigate) {
    ...
    parseQuery(queryString) {
        ...
        for (const pair of pairs) {
            ...
            if (!key)
                continue; // Bug #17: Skip if key is empty
    ...
    validateParameterTypes(params, types) {
    ...
    validateRequiredParameters(pattern, params) {
    ...
    buildCanonicalPath(pattern, params) {
    ...
    extractTypeConstraints(pattern) {
...
if (typeof window !== 'undefined') {
    window.SoftRouter = __router;
    window.__router = __router;
}
if (typeof globalThis !== 'undefined') {
    globalThis.__router = __router;
}
export { __router };
