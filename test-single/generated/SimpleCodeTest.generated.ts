...
export class SimpleCodeTest {
            ...
            if (!el1 || el1.tagName.toLowerCase() !== 'p') {
                ...
                if (container.children[0]) {
                    container.replaceChild(el1, container.children[0]);
                        ...
                        const el2 = document.createTextNode('Count: ');
        if (isFirstRender) {
            container.appendChild(el2);
        }
        let el3: Text;
        if (isFirstRender) {
            el3 = document.createTextNode(String(this.count));
            container.appendChild(el3);
        } else {
            el3 = container.childNodes[2] as Text;
            if (el3 && el3.nodeType === 3) {
                el3.textContent = String(this.count);
            }
        }
    ...
    container.appendChild(el1);
        ...
        let el4: HTMLButtonElement;
        if (isFirstRender) {
            el4 = document.createElement('button') as HTMLButtonElement;
        } else {
            el4 = container.children[3] as HTMLButtonElement;
            if (!el4 || el4.tagName.toLowerCase() !== 'button') {
                // Structure changed, rebuild this element
                el4 = document.createElement('button') as HTMLButtonElement;
        ...
        const handler_el4_click = (e) => {
            this.inc();
            this.__scheduleRender();
        };

        el4.addEventListener('click', handler_el4_click);
        const el5 = document.createTextNode('+');
        if (isFirstRender) {
            container.appendChild(el5);
        }
        this.__cleanup.push(() => el4.removeEventListener('click', handler_el4_click));
        if (isFirstRender) {
            container.appendChild(el4);
        }
