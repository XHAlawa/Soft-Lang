...
export class StateTest {
    ...
    set counter(value: any) {
        this._counter = value;
        // TODO: Trigger re-render on state change
    }
            ...
            if (!el1 || el1.tagName.toLowerCase() !== 'h3') {
                ...
                if (container.children[0]) {
                    container.replaceChild(el1, container.children[0]);
                        ...
                        const el2 = document.createTextNode('State Test');
        if (isFirstRender) {
            ...
            if (!el3 || el3.tagName.toLowerCase() !== 'p') {
                ...
                if (container.children[2]) {
                    container.replaceChild(el3, container.children[2]);
                        ...
                        const el4 = document.createTextNode('Counter: ');
        if (isFirstRender) {
            container.appendChild(el4);
        }
        let el5: Text;
        if (isFirstRender) {
            el5 = document.createTextNode(String(this.counter));
            container.appendChild(el5);
        } else {
            el5 = container.childNodes[4] as Text;
            if (el5 && el5.nodeType === 3) {
                el5.textContent = String(this.counter);
            }
        }
    ...
    container.appendChild(el3);
        ...
        let el6: HTMLButtonElement;
        if (isFirstRender) {
            el6 = document.createElement('button') as HTMLButtonElement;
        } else {
            el6 = container.children[5] as HTMLButtonElement;
            if (!el6 || el6.tagName.toLowerCase() !== 'button') {
                // Structure changed, rebuild this element
                el6 = document.createElement('button') as HTMLButtonElement;
