...
export class MultiEventTest {
            ...
            if (!el1 || el1.tagName.toLowerCase() !== 'h3') {
                ...
                if (container.children[0]) {
                    container.replaceChild(el1, container.children[0]);
                        ...
                        const el2 = document.createTextNode('Multiple Events Test');
        if (isFirstRender) {
    ...
    container.appendChild(el1);
        ...
        const handler_el3_mouseenter = (e) => {
            this.handleEnter();
            this.__scheduleRender();
        };

        el3.addEventListener('mouseenter', handler_el3_mouseenter);
        this.__cleanup.push(() => el3.removeEventListener('mouseenter', handler_el3_mouseenter));
        const handler_el3_mouseleave = (e) => {
            this.handleLeave();
            this.__scheduleRender();
        };

        el3.addEventListener('mouseleave', handler_el3_mouseleave);
        const el4 = document.createTextNode('Hover and click me\r\n  ');
        if (isFirstRender) {
            container.appendChild(el4);
        }
        this.__cleanup.push(() => el3.removeEventListener('mouseleave', handler_el3_mouseleave));
        if (isFirstRender) {
            container.appendChild(el3);
        }
        let el5: HTMLDivElement;
        if (isFirstRender) {
            el5 = document.createElement('div') as HTMLDivElement;
        } else {
            el5 = container.children[4] as HTMLDivElement;
            if (!el5 || el5.tagName.toLowerCase() !== 'div') {
                // Structure changed, rebuild this element
                el5 = document.createElement('div') as HTMLDivElement;
                if (container.children[4]) {
                    container.replaceChild(el5, container.children[4]);
                } else {
                        const el6 = document.createTextNode('Clicks: @clickCount\r\n    Enters: @enterCount\r\n    Leaves: @leaveCount\r\n  ');
        if (isFirstRender) {
