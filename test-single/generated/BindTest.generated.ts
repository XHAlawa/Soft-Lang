...
export class BindTest {
    ...
    public __render(container: HTMLElement): void {
        ...
        if (isFirstRender) {
            el1 = document.createElement('input') as HTMLInputElement;
        } else {
            el1 = container.children[0] as HTMLInputElement;
            if (!el1 || el1.tagName.toLowerCase() !== 'input') {
                // Structure changed, rebuild this element
                el1 = document.createElement('input') as HTMLInputElement;
        ...
        el1.setAttribute('data-focus-id', 'el1');
        el1.value = String(this.value ?? '');
        ...
        if (isFirstRender) {
            el2 = document.createElement('div') as HTMLDivElement;
        } else {
            el2 = container.children[1] as HTMLDivElement;
            if (!el2 || el2.tagName.toLowerCase() !== 'div') {
                // Structure changed, rebuild this element
                el2 = document.createElement('div') as HTMLDivElement;
                if (container.children[1]) {
                    container.replaceChild(el2, container.children[1]);
                } else {
                        const el3 = document.createTextNode('@value');
        if (isFirstRender) {
