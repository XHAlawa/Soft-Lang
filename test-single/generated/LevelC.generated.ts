...
export class LevelC {
    ...
    public __render(container: HTMLElement): void {
        ...
        if (isFirstRender) {
            el1 = document.createElement('h4') as HTMLHeadingElement;
        } else {
            el1 = container.children[0] as HTMLHeadingElement;
            if (!el1 || el1.tagName.toLowerCase() !== 'h4') {
                // Structure changed, rebuild this element
                el1 = document.createElement('h4') as HTMLHeadingElement;
                if (container.children[0]) {
                    container.replaceChild(el1, container.children[0]);
                } else {
                        const el2 = document.createTextNode('Level C');
        if (isFirstRender) {
    ...
    container.appendChild(el0);
        ...
        el0.setAttribute('class', 'level-c');
        if (isFirstRender) {
                let el1: HTMLHeadingElement;
        if (isFirstRender) {
            el1 = document.createElement('h4') as HTMLHeadingElement;
